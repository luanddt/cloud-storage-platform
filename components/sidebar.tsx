  ...
  return (
    <aside className="p-7 xl:w-81.25 lg:w-70 w-27 sm:flex flex-col gap-7 hidden overflow-auto">
        ...
        {navItems.map((item) => (
          <li key={item.name}>
            <Link
              href={item.url}
              className={cn(
                "hover:bg-primary lg:px-7.5 p-3.5 lg:rounded-full rounded-2xl shadow-drop-2 group flex items-center gap-4",
            ...
            >
              ...
              <h5 className={cn("h5 group-hover:text-primary-foreground max-lg:hidden", pathname === item.url && "text-primary-foreground")}>
                {item.name}
              </h5>
            </Link>
          </li>
      ...
      <Image
        ...
        width={252}
        height={208}
        className="w-full h-52 max-lg:hidden"
      />
